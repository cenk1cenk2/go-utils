...
func runCommands(task *Task, commands []Command) {
	for _, command := range commands {
		...
		command.Args = utils.DeleteEmptyStringsFromSlice(command.Args)
		...
		if exiterr, ok := err.(*exec.ExitError); ok {
			if status, ok := exiterr.Sys().(syscall.WaitStatus); ok {
				Log.WithField("context", "EXIT").
					Debugln(fmt.Sprintf("$ %s > Exit Code: %v", cmd, status.ExitStatus()))
			}
		}
...
func CreateCommandReaders(
	...
	if err != nil {
		return nil, nil, nil, nil,
			fmt.Errorf("Failed creating command stdout pipe: %s", err)
	}
	...
	if err != nil {
		return nil, nil, nil, nil,
			fmt.Errorf("Failed creating command stderr pipe: %s", err)
	}
